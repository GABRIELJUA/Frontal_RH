...
loadEmployees(): void {
    ...
    next: data => {
     ...
     // Filtro seguro
      const soloEmpleados = data.filter(
        e => e.rol && e.rol.toUpperCase() === 'EMPLEADO'
      );

      this.employeesOriginal = soloEmpleados;
      this.employees = soloEmpleados;

      this.departamentos = [...new Set(
        soloEmpleados.map(e => e.departamento)
      )];
  ...
  filtrarEmpleadosd() {
    this.employees = this.employeesOriginal.filter(e => {

      const coincideBusqueda =
        e.nombre.toLowerCase().includes(this.searchTerm.toLowerCase()) ||
        e.puesto.toLowerCase().includes(this.searchTerm.toLowerCase());

      const coincideDepartamento =
        this.departamentoSeleccionado === '' ||
        e.departamento === this.departamentoSeleccionado;
  ...
  this.employees = this.employeesOriginal.filter(e => {
    ...
    const coincideBusqueda =
      ...
      e.apellido_paterno.toLowerCase().includes(texto) ||
